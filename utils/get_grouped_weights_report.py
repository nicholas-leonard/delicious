...
def get_weights_report(model_path = None, model = None, rescale = 'individual', border = False, norm_sort = False,
    ...
    W1 = W1.T
    W0 = W0.transpose(1,2,0)
    h1 = W1.shape[0]
    h0 = W0.shape[0]*W0.shape[1]
    print W0.shape, W1.shape

    weights_view1 = dataset.get_weights_view(W1)
    weights_view0 = []
    for i in range(W0.shape[0]):
        weights_view0.append(dataset.get_weights_view(W0[i,:]))
    assert weights_view1.shape[0] == h1
    ...
    pv0 = patch_viewer.PatchViewer(grid_shape=(hr0,hc0), patch_shape=weights_view0[0].shape[1:3],
            is_color = weights_view0[0].shape[-1] == 3)
